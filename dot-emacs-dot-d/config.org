...
* Setup
To quickly get up and running, we can generate the init files like so:
#+begin_src sh :tangle no
  emacs -Q ~/.emacs.d/config.org -f org-babel-tangle -f kill-emacs
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Apply font styles for child frames created from daemon
  (add-hook 'after-make-frame-functions
            #'(lambda (frame)
              (with-selected-frame frame
                (aa/apply-org-font-styles))))

  ;; Apply font styles when run as standalone without daemon
  (if window-system (aa/apply-org-font-styles))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-hook 'after-make-frame-functions
            (lambda (frame)
              (with-selected-frame frame
                (aa/apply-org-font-customisation))))

  (if window-system (aa/apply-org-font-customisation))
#+END_SRC
